...
def update_historical_weather_data(target_year, batch_size=10):
    ...
    for index, row in grid_df.iterrows():
        ...
        if os.path.exists(meta_file):
            ...
            if time.time() - last_update < 24 * 3600:
                logging.info(f"[{target_year}] {file_name} 在 24 小時內已更新，跳過。")
                continue
        points_to_update.append((lat, lon))
        meta_files[(lat, lon)] = meta_file
    if not points_to_update:
    ...
    for i, response in enumerate(hist_responses):
        try:
            df = process_historical_response(response)
            hist_data_dict[(lats[i], lons[i])] = df
            logging.info(f"[{target_year}] 歷史資料處理成功：({lats[i]}, {lons[i]})，筆數：{len(df)}")
        except Exception as e:
            logging.error(f"[{target_year}] 處理歷史資料失敗：({lats[i]}, {lons[i]})，錯誤：{e}")
    
    for point in points_to_update:
        lat, lon = point
        if point not in hist_data_dict:
            logging.error(f"[{target_year}] 缺少歷史資料：({lat}, {lon})")
            continue
        hist_df = hist_data_dict[point]
        cum_vars = ["temperature_2m_mean", "apparent_temperature_mean", "daylight_duration",
                    "sunshine_duration", "precipitation_sum", "rain_sum", "precipitation_hours",
...
# 模式: update (更新當年資料), forecast (依當年資料預報),
#       update_historical (更新指定歷史年度資料),
#       forecast_historical (依歷史資料預報)
# =============================================================================
def main():
